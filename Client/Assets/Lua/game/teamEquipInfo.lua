...
--==================================================================================================
teamEquipInfo.assets=
{
   Asset("teamEquipInfo.u3d", {"Config"}),
}
-- 检查界面是否打开
teamEquipInfo.isShow = false;
-- panel
teamEquipInfo.panels = {};
-- 当前选中的英雄id
teamEquipInfo.selIndex = 1;
-- 当前英雄的装备栏状态， 装备/未装备等等
teamEquipInfo.weaponStates = {};
-- 当前点击的装备栏位置
teamEquipInfo.weaponSel = 0;
-- 当前的装备弹出框状态。 为空表示没有子窗口弹出
teamEquipInfo.popStates = {};
--- 当前界面状态
teamEquipInfo.panelState = 1;

teamEquipInfo.teamData = {}
------------------private-----------------
local MTeamLen = 5;
...
------------------public------------------
function teamEquipInfo:getMHeadPanel()
...
function teamEquipInfo:getMHeroPanel()
...
function teamEquipInfo:getMInfoPanel()
...
function teamEquipInfo:getPopPanel()
...
function teamEquipInfo:InitMainPan()
  -------- 主装备面板
  --- 头像栏
  self:getMHeadPanel();
  -- 英雄装备栏
...
--- 设置默认选中的头像
function teamEquipInfo:setDefaultSelIndex()
	self.selIndex = 1;
	for i=1, MTeamLen do
		if self.teamData.data.team[i] then
			self.selIndex = i;
			return;
		end
	end
...
--- 显示头像 view
function teamEquipInfo:updateHeadPanel()
    ...
    for i, subView in ipairs(self.mHeadViews) do
    	...
    	hData = self.teamData.data.team[i]
    	if hData then
    		--- 有英雄上阵
			local localHData = Model.units[tostring(hData.system_hero_id)];
...
--- 检查对应装备可装备状态
function teamEquipInfo:checkWeaponState(goods_id)
	local goodModel = localPack:getGoodDataById(goods_id);
...
function teamEquipInfo:setWeaponShow(wIndex, hData)
	--- 若英雄当前位置有装备(则 > 0)
 	if table.getn(hData.player_hero_equip) < wIndex then
 		return
 	end

	local curGood = hData.player_hero_equip[wIndex].id;
	local localHData = Model.units[tostring(hData.system_hero_id)];
	--- 玩家等级
	local colorlv = hData.player_hero_color;
	--- view 初始化
...
--- 显示英雄当前的状态
function teamEquipInfo:updateHeroPanel(data)
 	print("传入装22222")
-- 	printTable(data)
	local localHData = Model.units[tostring(data.unitCfg.id)];
	self.mHeroViews.name.text = getValue(localHData.name);
	self.mHeroViews.lv.text = "Lv:" .. tostring(data.player_hero_lv);
	local nextExp = data.player_hero_next_exp;
	if nextExp == 0 then nextExp = 99999 end
	self.mHeroViews.exp.value = 1.0 * data.player_hero_exp/nextExp;
	self.mHeroViews.expLable.text = tostring(data.player_hero_exp) .. "/" .. nextExp;
	---- 装备栏
	self.weaponStates = {};
	for wIndex = 1, #self.mHeroViews.weapons do
		self:setWeaponShow(wIndex, data);
	end
end

--- 显示英雄当前的信息
function teamEquipInfo:updateInfoPanel()
    local hData = self.teamData.data.team[self.selIndex]
	...
	--- hp/防御/攻击/速度
	SetHeroNormalInfo(self.mHeroInfo.hp, hData.player_hero_attribute.maxHp);
	local text = string.format("%.2f", hData.player_hero_attribute.defend);
	SetHeroNormalInfo(self.mHeroInfo.defence, text);
	text = string.format("%.2f", hData.player_hero_attribute.damage);
	SetHeroNormalInfo(self.mHeroInfo.attack, text);
	text = string.format("%.2f", hData.player_hero_attribute.speed);
	SetHeroNormalInfo(self.mHeroInfo.speed, text);
end

--- 当收到新消息后， 根据当前的状态更新显示
function teamEquipInfo:updateViewsByState()
	if self.panelState == PanelStates["normal"] then
		---- 基本的初始化
		self:setDefaultSelIndex();
		self:updateHeadPanel();
	  	self:updateHeroPanel(self.teamData.data.team[self.selIndex]);
...
---- 英雄列表更新消息(副)
function teamEquipInfo.getHeroDatas(netData)
	--- 避免接收消息后刷新队伍界面
	if localTeam.isShow then return end
    -------------- data handle
  	localTeam.getHeroDatas(netData);
 	---------------- view handle
 	if teamEquipInfo.isShow then teamEquipInfo:updateViewsByState() end
end

--- 获得背包物品变化消息(副)
function teamEquipInfo.getGoodsChange(netData)
	--- 避免接收消息后刷新商品界面
...
-------------------- show/hide
function teamEquipInfo:showAll(teamTable)

	self.teamData.data = teamTable
	print("传入装备界面")
	printTable(self.teamData.data)
...
function teamEquipInfo:hideAll()
...
----------------------------------- system call
-- 资源加载完成时候调用方法
function teamEquipInfo:onAssetsLoad(items)
...
-- 第一次注册的时候调用
function teamEquipInfo:initialize()
...
--- 每次显示都会调用
function teamEquipInfo:onShowed( ... )
...
--- 当flag为true的时候， 关闭碰撞块
function teamEquipInfo:setMainPopBtn(state, flag, color)
...
--- 根据点击的装备栏 初始化弹出表
function teamEquipInfo:showPopPanel(weaponData)
...
function teamEquipInfo:handlePopTitle(index, goods_id, isShow)
...
--- 设置合成界面的显示
function teamEquipInfo:setComposeShow(goods_id)
	--- 获得物品的本地数据
...
--- 检查当数据变化时， 更新子 弹出框
--- true 是增加， false是减少
function teamEquipInfo:updateSubPopPanel(goods_id, addSwitch)
	local len = #self.popStates;
	--- 关闭窗口
...
--- 检查是否可以升级
function teamEquipInfo:checkHeroWearAll()
	local len = 0;
...
--- 点击主弹出框的按钮响应
function teamEquipInfo:checkMainPopBtn(weaponData)
	--- 确定[已装备]
...
--- 检查子合成item
function teamEquipInfo:checkSubItem(index)
	--- 获得合成列表的尾部商品的合成信息
...
--- 返回到某子合成窗口
function teamEquipInfo:backSubItem(index)
...
function teamEquipInfo:popOnClick(obj)
	local cmd = obj.name;
...
function teamEquipInfo:setPanelState(state)
	self.panelState = PanelStates[state];
end

--- 点击对应的装备按钮
function teamEquipInfo:weaponOnClick(obj)
	local cmd =obj.name;
	local tran = obj.transform.parent;
...
function teamEquipInfo:onClick(obj,arg)
  local cmd =obj.name;
  local tran = obj.transform.parent;
