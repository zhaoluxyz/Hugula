...
local enemyHeroAtk=class(function(self)
	self.brain=LuaBrain(self.onStart)
end)

function enemyHeroAtk.onStart(actor)
    	local root = SelectorNode("enemyHeroAtk")
         root:AddNode(BrainEventManager:EventSkillAttack()) --技能攻击
         root:AddNode(BrainEventManager:EventMoveAttack()) --技能攻击
         root:AddNode(BrainEventManager:EventMoveTo()) --技能攻击
         root:AddNode(BrainEventManager:EventFindNearestEnemyInRangeVisible()) --技能攻击
         root:AddNode(BrainEventManager:EventFindNearestEnemyFromAll()) --技能攻击
         return root
